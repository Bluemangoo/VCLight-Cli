...
const keys = {
    ...
    httpServer: "VCLight http server"
};
...
module.exports = async function(name) {
    ...
    let question = [{
    ...
    },
        {
            ...
            choices: [keys.prettier, keys.gitignore, keys.httpServer]
        }
    ];

    let presetChosen, featuresChosen;
    ...
    let devDependencies = ["vercel", "@types/node", "@vercel/node"];
    ...
    if (template.httpServer) {
        devDependencies.push("ts-node", "typescript");
    }
    ...
    new Promise(() => {
        setTimeout(async () => {
            ...
            if (template.httpServer) {
                packageJson.scripts.serve = "ts-node serve";
            }
            let taskList = [];
            let deps = {};
            let devDeps = {};
